use crate::near_vm_runner::logic;
use logic::dependencies::{Result, TrieNodesCount};
use logic::types::ReceiptIndex;
use logic::{External, StorageGetMode, ValuePtr};
...
impl ValuePtr for MockedValuePtr {
    ...
    fn deref(&self) -> logic::dependencies::Result<Vec<u8>> {
        Ok(self.value.clone())
    }
}
...
impl External for MockedExternal {
    ...
    fn create_action_receipt(
        ...
        receipt_indices: Vec<ReceiptIndex>,
        receiver_id: AccountId,
    ) -> Result<ReceiptIndex, logic::VMLogicError> {
    ...
    fn create_promise_yield_receipt(
        &mut self,
        receiver_id: AccountId,
    ) -> Result<(ReceiptIndex, CryptoHash), logic::VMLogicError> {
        let index = self.action_log.len();
    ...
    fn submit_promise_resume_data(
        ...
        data_id: CryptoHash,
        data: Vec<u8>,
    ) -> Result<bool, logic::VMLogicError> {
