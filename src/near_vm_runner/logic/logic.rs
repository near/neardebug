...
use crate::near_vm_runner::logic::context::VMContext;
use crate::near_vm_runner::logic::errors::{HostError, VMLogicError};
use crate::near_vm_runner::ProfileDataV3;
...
impl ExecutionResultState {
    ...
    fn check_can_add_a_log_message(&self) -> Result<()> {
        if self.logs.len() as u64 >= self.config.limit_config.max_number_logs {
            Err(HostError::NumberOfLogsExceeded {
                limit: self.config.limit_config.max_number_logs,
            }
            .into())
        } else {
            Ok(())
...
impl VMLogic {
    ...
    pub(crate) fn registers(&mut self) -> &mut super::vmstate::Registers {
        &mut self.registers
    }

    pub(crate) fn memory(&mut self) -> &mut super::vmstate::Memory {
        &mut self.memory
    }
    ...
    pub(crate) fn get_utf8_string_free(&mut self, len: u64, ptr: u64) -> Result<String> {
        let mut buf;
        let max_len = self.config.limit_config.max_total_log_length;
        if len != u64::MAX {
            buf = self
                .memory
            ...
            for i in 0..=max_len {
                // self.memory_get_u8 will check for u64 overflow on the first iteration (i == 0)
                let el = self.memory.view_for_free(MemSlice {
                    ptr: ptr + i,
                    len: 1,
                })?;
                if el[0] == 0 || i == max_len {
    ...
    fn get_public_key(&mut self, ptr: u64, len: u64) -> Result<PublicKeyBuffer> {
        Ok(PublicKeyBuffer::new(&get_memory_or_register!(
            self, ptr, len
        )?))
    }
    ...
    pub fn ecrecover(
        ...
        let signature = {
            ...
            if v < 4 {
                ...
                return Err(VMLogicError::HostError(HostError::ECRecoverError {
                    msg: format!(
                        "V recovery byte 0 through 3 are valid but was provided {}",
                        v
                    ),
                }));
            }
    ...
    pub fn panic_utf8(&mut self, len: u64, ptr: u64) -> Result<()> {
        self.result_state.gas_counter.pay_base(base)?;
        Err(HostError::GuestPanic {
            panic_msg: self.get_utf8_string(len, ptr)?,
        }
        .into())
    }
    ...
    pub fn abort(&mut self, msg_ptr: u32, filename_ptr: u32, line: u32, col: u32) -> Result<()> {
        ...
        let filename_len = self.memory.get_u32(
            &mut self.result_state.gas_counter,
            (filename_ptr - 4) as u64,
        )?;
